...
export function VehicleDetailPage() {
  ...
  useEffect(() => {
    ...
    getVehicle(id)
      .then(setVehicle)
      .catch(() => setError('Araç detay bilgileri yüklenemedi.'))
  ...
  const openEdit = () => {
    ...
    setForm({
      ...
      kilometer: String(vehicle.kilometer),
      lastInspectionDate: toDateInputValue(vehicle.lastInspectionDate),
  ...
  if (error || !vehicle) {
    return (
      <div className="animate-slide-in-right space-y-4">
        <button type="button" className="btn-secondary" onClick={() => navigate('/admin/vehicles')}>
          Geri dön
        </button>
        <div className="page-card text-sm text-red-600">{error || 'Araç bulunamadı.'}</div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-slide-in-right space-y-5">
      ...
      <section className="page-card space-y-5">
        <div>
          <p className="text-sm font-medium text-slate-500">Araç Detayı</p>
          <h1 className="mt-1 text-2xl font-semibold text-slate-950">{vehicle.plate}</h1>
        ...
        <div className="grid grid-cols-1 gap-3 text-sm md:grid-cols-2">
          <DetailItem label="Plaka" value={vehicle.plate} />
          <DetailItem label="Marka" value={vehicle.brand} />
          <DetailItem label="Model" value={vehicle.model} />
          <DetailItem label="Model Yılı" value={String(vehicle.modelYear)} />
          <DetailItem label="Kilometre" value={`${vehicle.kilometer.toLocaleString('tr-TR')} km`} />
          <DetailItem label="Son Muayene Tarihi" value={formatDate(vehicle.lastInspectionDate)} />
          <DetailItem label="Durum" value={vehicle.status === 'ACTIVE' ? 'Aktif' : 'Pasif'} />
        </div>
      </section>

      {isEditOpen && form ? (
        <FormModal title="Aracı Düzenle" onClose={() => setIsEditOpen(false)}>
          <form onSubmit={onSave} className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button className="btn-secondary" type="button" onClick={() => setIsEditOpen(false)}>
                Vazgeç
              </button>
            </div>
          </form>
