...
async function updateProfile(userId, payload) {
  ...
  if (!firstName || !lastName) {
    throw new ApiError(400, 'Ad ve soyad zorunludur.');
  }

  const updates = {
    firstName,
    lastName,
    'businessCard.displayName': `${firstName} ${lastName}`.trim()
  };

  if (Object.prototype.hasOwnProperty.call(payload, 'birthDate')) {
    updates.birthDate = payload.birthDate ? new Date(payload.birthDate) : null;
  }
